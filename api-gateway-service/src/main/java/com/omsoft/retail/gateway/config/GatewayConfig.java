...
public class GatewayConfig {
    ...
    public RouteLocator customRoutes(RouteLocatorBuilder builder) {
        return builder.routes()
                ...
                .route("payment-swagger", r -> r.path("/api/payment/v3/api-docs/**", "/api/payment/swagger-ui/**")
                        .filters(f ->  f.filter(jwtAuthenticationFilter))
                        .uri("lb://payment-management-service"))
                .route("inventory-service", r -> r.path("/api/inventory/**")
                        .filters(f -> f.filter(jwtAuthenticationFilter))
                        .uri("lb://inventory-management-service"))
                .route("inventory-swagger", r -> r.path("/api/inventory/v3/api-docs/**", "/api/inventory/swagger-ui/**")
                        .filters(f ->  f.filter(jwtAuthenticationFilter))
                        .uri("lb://inventory-management-service"))
                .route("order-service", r -> r.path("/api/order/**")
                        .filters(f -> f.filter(jwtAuthenticationFilter))
                        .uri("lb://order-management-service"))
                .route("order-swagger", r -> r.path("/api/order/v3/api-docs/**", "/api/order/swagger-ui/**")
                        .filters(f ->  f.filter(jwtAuthenticationFilter))
                        .uri("lb://order-management-service"))
